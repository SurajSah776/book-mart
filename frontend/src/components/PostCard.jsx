...
const PostCard = ({ post }) => {
  ...
  return (
    <div className="background rounded-lg shadow-md mb-4 border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-200 flex flex-col h-full min-h-[400px]">
      ...
      <div className="p-3 flex items-center justify-between border-b border-gray-200">
        <div className="flex items-center space-x-3">
          ...
          >
            <div className="w-8 h-8 rounded-full background flex items-center justify-center overflow-hidden">
              {post.profilePic ? (
                <img
          ...
          <div className="flex flex-col text-color">
            <Link
              to={`/profile/${post.user._id}`}
              className="font-semibold hover:secondary text-sm"
            >
        ...
        <button
          className="text-gray-500 hover:background p-1.5 rounded-full transition-colors"
      ...
      <Link
        to={`/posts/${post._id}`}
        className="p-3 flex-grow block hover:background transition-colors"
      >
        {/* Book Description */}
        {post.description && (
          <p className="mb-2 text-color text-sm line-clamp-2">
            {post.description}
          </p>
        )}

        {/* Book Image */}
        <div className="mb-2 rounded-lg overflow-hidden border border-gray-200 h-48 flex items-center justify-center bg-gray-100">
          <img
            src={
              post.image
        ...
        <div className="space-y-1.5 text-color">
          <h3 className="font-bold text-base text-color line-clamp-1">
            {post.bookName}
          </h3>
      ...
      <div className="px-3 pb-3 mt-auto">
        {shouldShowRequestButton() ? (
          ...
          <div className="text-center py-1.5 text-xs text-gray-700 bg-gray-100 rounded">
            This book has been donated
          </div>
        ) : !currentUser ? (
          <div className="text-center py-1.5 text-xs text-gray-700">
            <Link to="/login" className="accent hover:secondary hover:underline">
              Log in
            </Link>{" "}
